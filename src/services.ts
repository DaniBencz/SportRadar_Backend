...
const getTournamentNamesAndIDs = async (uri: string): Promise<{ [key: string]: string }> => {
  ...
  const tournamentNamesAndIDs = tournamentsData.reduce((acc: { _id: number, name: string }, cur: { _id: number, name: string }) => ({ ...acc, ...{ [cur._id]: cur.name } }), {});
...
const filterMatchesDataFromTournamentData = async (detailedTournamentData: Promise<any>[]): Promise<{ [key: string]: IMatchRaw }[]> => (await Promise.all(detailedTournamentData)).reduce((acc, cur) => {
...
const pushAllMatchesInOneSingleArray = (matchesGroupedByTournament: { [key: string]: IMatchRaw }[]) => matchesGroupedByTournament.reduce((acc: IMatchRaw[], cur: { [key: string]: IMatchRaw }) => [...acc, ...convertMatchesObjectToArray(cur)], []);
